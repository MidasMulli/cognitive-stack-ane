...
├── entities/
│   ├── llama-3.1-8b.md         # Auto-generated profile at top
│   ├── ane.md
│   └── ...
├── facts/
├── decisions/
...
**CPU tier (12 tests):** Fact extraction from a session-log paragraph (all 4 types), entity detection (model names, dispatch counts, hardware components), semantic recall accuracy, type filtering, noise rejection.

**ANE tier (5 tests):** Entity summarization (Llama-8B profile), relationship extraction (4 entities), constraint identification (dispatch floor cliff), quantity extraction (4 measurements from a benchmark paragraph). All under 2 seconds. Requires ANE server running (`ane_server.py`).
